...
import cn.dreampie.route.RestyIniter;
 ...
 */
public final class MyRestFilter implements Filter {

	public static final String PARAM_NAME_CONFIGCLASS = "configClass";
	public static final String PARAM_NAME_EXCLUSIONS = "exclusions";
	private static final Logger logger = Logger.getLogger(MyRestFilter.class);
	...
	public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain chain) throws IOException, ServletException {
		if (!(servletRequest instanceof HttpServletRequest) || !(servletResponse instanceof HttpServletResponse)) {
			throw new ServletException("Rest doesn't support non-HTTP request or response.");
		}
		servletRequest.setCharacterEncoding(encoding);
		servletResponse.setCharacterEncoding(encoding);
		MyHttpRequest request = new MyHttpRequest((HttpServletRequest) servletRequest, restyIniter.getServletContext());
		...
		if (!isExclusion(request.getRestPath())) {
			try {
				RequestContext context = new MyRequestContext(request);
